...
public class CustomDefaultTokenServices implements AuthorizationServerTokenServices, ResourceServerTokenServices,
	...
	public OAuth2AccessToken createAccessToken(OAuth2Authentication authentication) throws AuthenticationException {
		...
		try {
			...
			if ( entAccessToken != null ) {
				...
				existingAccessToken = deserializeAccessToken( entAccessToken.getToken() );
		...
		if (accessToken.getRefreshToken() != null) {
			refreshTokenStr = accessToken.getRefreshToken().getValue();
		}

		entAccessToken.setTokenId( extractTokenKey(accessToken.getValue()) );
		entAccessToken.setToken( serializeAccessToken(accessToken) );
		...
		entAccessToken.setRefreshToken( extractTokenKey(refreshTokenStr) );
				
		accessTokenRepository.save( entAccessToken );
		...
		if (refreshToken != null) {
			...
			RefreshTokenEntity entRefreshToken = new RefreshTokenEntity ();
			...
			refreshTokenRepository.save( entRefreshToken );
	...
	public OAuth2AccessToken refreshAccessToken(String refreshTokenValue, TokenRequest tokenRequest)
		...
		RefreshTokenEntity entRefreshToken = refreshTokenRepository.findBytokenId( extractTokenKey(refreshTokenValue) );
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("Failed to find refresh token for token " + refreshTokenValue);
			}
		}
		catch (IllegalArgumentException e) {
			logger.warn("Failed to deserialize refresh token for token " + refreshTokenValue, e);
		...
		try {
			...
			if (logger.isInfoEnabled()) {
				logger.info("Failed to find access token for token " + refreshToken.getValue());
			}
		}
		catch (IllegalArgumentException e) {
			logger.warn("Failed to deserialize access token for " + refreshToken.getValue(), e);
...
//		OAuth2Authentication authentication = tokenStore.readAuthenticationForRefreshToken(refreshToken);
	...
	public OAuth2Authentication loadAuthentication(String accessTokenValue) throws AuthenticationException,
		...
		AccessTokenEntity entAccessToken = null;

		try {

		    entAccessToken = accessTokenRepository.findByTokenId( extractTokenKey(accessTokenValue) );
			...
			if (logger.isInfoEnabled()) {
				logger.info("Failed to find access token for token " + accessTokenValue);
			}
		}
		catch (IllegalArgumentException e) {
			logger.warn("Failed to deserialize access token for " + accessTokenValue, e);
...
//		OAuth2AccessToken accessToken = tokenStore.readAccessToken(accessTokenValue);
		...
		try {
			...
			if (entAccessToken != null) {
				result =  deserializeAuthentication( entAccessToken.getAuthentication() );
			}
			...
			if (logger.isInfoEnabled()) {
				logger.info("Failed to find access token for token " + accessToken.getValue());
			}
		}
		catch (IllegalArgumentException e) {
			logger.warn("Failed to deserialize authentication for " + accessToken.getValue(), e);
